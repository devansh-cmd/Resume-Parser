 ...
 */
function parseRawResume(rawText) {
 ...
 */
function parseStructuredResume(data) {
    return {
 ...
 */
function findInstitution(text, patterns) {
...
function extractYear(text, pattern) {
    const match = text.match(pattern);
    return match ? parseInt(match[0]) : undefined;
}
function extractField(degree) {
    const fieldPattern = /(?:in|of)\s+([A-Z][a-z\s]+)/i;
    const match = degree.match(fieldPattern);
    return match ? match[1] : undefined;
}
function findCompany(text, pattern) {
    const match = text.match(pattern);
