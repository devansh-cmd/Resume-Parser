import { ParsedResume, ExtractedFeatures, Skill, WorkExperience, Education } from '../types';
...
export class FeatureExtractor {
   ...
   */
  public extractFeatures(parsedResume: ParsedResume): ExtractedFeatures {
    return {
   ...
   */
  private checkRequiredSkills(skills: Skill[]): boolean {
   ...
   */
  private checkPreferredSkills(skills: Skill[]): boolean {
    ...
    const candidateSkillNames = skills.map(skill => skill.name.toLowerCase());
    const hasPreferredSkills = preferredSkills.some(skill => 
      candidateSkillNames.includes(skill.toLowerCase())
    );
   ...
   */
  private checkExperienceRequirement(workExperience: WorkExperience[]): boolean {
   ...
   */
  private checkEducationRequirement(education: Education[]): boolean {
   ...
   */
  private calculateSkillMatch(skills: Skill[]): number {
    const allSkills = [
   ...
   */
  private determineEducationLevel(education: Education[]): string {
    if (education.length === 0) return 'high_school';
   ...
   */
  public extractDetailedFeatures(parsedResume: ParsedResume): any {
    return {
      // Basic info
   ...
   */
  private analyzeExperience(workExperience: WorkExperience[]): any {
    const totalMonths = this.calculateTotalExperience(workExperience);
   ...
   */
  private hasRecentExperience(workExperience: WorkExperience[]): boolean {
   ...
   */
  private analyzeCareerProgression(workExperience: WorkExperience[]): string {
    if (workExperience.length < 2) return 'insufficient_data';
   ...
   */
  private getTechnicalSkills(skills: Skill[]): string[] {
    return skills
   ...
   * Get soft skills
   */
  private getSoftSkills(skills: Skill[]): string[] {
    return skills
   ...
   */
  private analyzeEducation(education: Education[]): any {
    return {
   ...
   */
  private hasRelevantField(education: Education[]): boolean {
   ...
   */
  private calculateCompleteness(parsedResume: ParsedResume): number {
    let score = 0;
    const maxScore = 100;

    // Basic info (20 points)
    if (parsedResume.fullName !== 'Unknown') score += 10;
    if (parsedResume.email) score += 5;
    if (parsedResume.phone) score += 5;

    // Experience (30 points)
    if (parsedResume.workExperience.length > 0) score += 30;

    // Education (20 points)
    if (parsedResume.education.length > 0) score += 20;

    // Skills (20 points)
    if (parsedResume.skills.length > 0) score += 20;
