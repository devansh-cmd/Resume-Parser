import { ParsedResume, JobRequirements, ScreeningResult, ExtractedFeatures } from '../types';
...
export class ResumeEvaluator {
   ...
   */
  private extractEvaluationFeatures(candidateData: ParsedResume): any {
    return {
      // Basic validation
   ...
   */
  private performEvaluation(features: any, requirements: JobRequirements): any {
   ...
   */
  private hasValidContact(candidateData: ParsedResume): boolean {
  ...
  private hasRelevantExperience(workExperience: any[]): boolean {
  ...
  private analyzeSkillMatch(skills: any[]): any {
    const allSkills = [
  ...
  private hasRequiredSkills(skills: any[]): boolean {
  ...
  private hasRelevantEducation(education: any[]): boolean {
  ...
  private calculateCompleteness(candidateData: ParsedResume): number {
    let score = 0;
    const maxScore = 100;

    // Basic info (20 points)
    if (candidateData.fullName !== 'Unknown') score += 10;
    if (candidateData.email) score += 5;
    if (candidateData.phone) score += 5;

    // Experience (30 points)
    if (candidateData.workExperience.length > 0) score += 30;

    // Education (20 points)
    if (candidateData.education.length > 0) score += 20;

    // Skills (20 points)
    if (candidateData.skills.length > 0) score += 20;
